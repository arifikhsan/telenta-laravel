							...
							<div class="row mb-2"> 
								...
								<div class="col-md-6">
									<div class="form-group">
										<input id="name" class="form-control mb-3" type="text" placeholder="client name.. " >
									</div>
...
<script type="text/javascript">
	var titleAdd = "Add Client";
	var titleEdit = "Update Client";
	...
	$(document).ready(function() {
		table = $('#table').DataTable({
			...
			"ajax": {
				"url": "{{ url('dashboard/clients/fetch') }}",
				"type": "POST",
				"headers": {
	...
	function edit(id) {
		...
		var url = "{{ url('dashboard/clients/show') }}";
		httpGet(url, data);
	}
	...
	function save() {
		...
		if (method == 'add') {
			url = "{{ url('dashboard/clients/store') }}";
		} else if(method == 'update') {
			url = "{{ url('dashboard/clients/update') }}";
		} else {
			url = "{{ url('dashboard/clients/destroy') }}";
		}
