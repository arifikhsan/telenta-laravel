							...
							<div class="row mb-2"> 
								...
								<div class="col-md-6">
									<div class="form-group">
										<input id="name" class="form-control mb-3" type="text" placeholder="position name.. " >
									</div>
...
<script type="text/javascript">
	var titleAdd = "Add Position";
	var titleEdit = "Update Position";
	...
	$(document).ready(function() {
		...
		table = $('#table').DataTable({
			...
			"ajax": {
				"url": "{{ url('dashboard/positions/fetch') }}",
				"type": "POST",
				"headers": {
	...
	function edit(id) {
		...
		var url = "{{ url('dashboard/positions/show') }}";
		httpGet(url, data);
	}
	...
	function save() {
		...
		if (method == 'add') {
			url = "{{ url('dashboard/positions/store') }}";
		} else if(method == 'update') {
			url = "{{ url('dashboard/positions/update') }}";
		} else {
			url = "{{ url('dashboard/positions/destroy') }}";
		}
