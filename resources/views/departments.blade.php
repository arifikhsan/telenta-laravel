							...
							<div class="row mb-2"> 
								...
								<div class="col-md-6">
									<div class="form-group">
										<input id="name" class="form-control mb-3" type="text" placeholder="department name.. " >
									</div>
...
<script type="text/javascript">
	var titleAdd = "Add Department";
	var titleEdit = "Update Department";
	...
	$(document).ready(function() {
		table = $('#table').DataTable({
			...
			"ajax": {
				"url": "{{ url('dashboard/departments/fetch') }}",
				"type": "POST",
				"headers": {
	...
	function edit(id) {
		...
		var url = "{{ url('dashboard/departments/show') }}";
		httpGet(url, data);
	}
	...
	function save() {
		...
		if (method == 'add') {
			url = "{{ url('dashboard/departments/store') }}";
		} else if(method == 'update') {
			url = "{{ url('dashboard/departments/update') }}";
		} else {
			url = "{{ url('dashboard/departments/destroy') }}";
		}
